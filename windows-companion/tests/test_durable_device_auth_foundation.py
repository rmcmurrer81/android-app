...
def test_module_is_staged_and_unreferenced_by_current_runtime_and_artifacts() -> None:
    ...
    for path in required_runtime_paths:
        assert path.is_file(), f"required runtime source is missing: {path.name}"
        assert module_name not in path.read_text(encoding="utf-8")

    # PyInstaller .spec files are generated by CI and are not required to be
    # checked into source. If a developer has generated them locally, keep the
    # same staged-auth isolation assertion without failing merely because a
    # clean checkout has no generated build files yet.
    for path in (
