...
ACTIVE_WORKFLOW_NAMES = (
    ...
    "sarah-team-integration-package.yml",
)
...
class ArtifactCredentialBoundaryTest(unittest.TestCase):
    ...
    def test_android_uses_one_keystore_backed_owner_activation(self):
        ...
        self.assertIn('BuildConfig.APPLICATION_ID.endsWith(".eventcandidate")', model)
        self.assertIn("return clean(BuildConfig.SARAH_MODEL_BACKEND_TOKEN)", model)
        self.assertIn('return "";', model)
    ...
    def test_active_generic_and_final_builds_have_no_job_level_credentials(self):
        ...
        voice_gate = step(generic, "Verify paid voice is not required")
        self.assertIn("no paid voice service", voice_gate)
        self.assertIn("SarahLocalVoice", voice_gate)
        self.assertNotIn("secrets.SARAH_ELEVENLABS_API_KEY", generic)
        self.assertNotIn("api.elevenlabs.io", generic)
