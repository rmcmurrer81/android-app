...
public final class ProactiveDiscoveryScheduler {
    ...
    public static boolean runSoon(Context c){ JobScheduler js=(JobScheduler)c.getSystemService(Context.JOB_SCHEDULER_SERVICE); if(js==null)return false;
        return js.schedule(new JobInfo.Builder(JOB+1,new ComponentName(c,ProactiveDiscoveryJobService.class)).setRequiredNetworkType(JobInfo.NETWORK_TYPE_ANY).setMinimumLatency(3000).setOverrideDeadline(30000).build())==JobScheduler.RESULT_SUCCESS; }
